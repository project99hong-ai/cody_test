...
| 운영 로그 키 정화 | 로그 정화 변경이 배포된 뒤 같은 유형의 요청에서 Vercel 오류 로그 확인 | PASS — 키 부분 문자열 대신 `[REDACTED]` 표시 |
...
키 로그 정화 변경 배포 후 2026-09-24에 합성 입력으로 재검증했습니다. 호출은 같은 인증 실패로 `502`였으며, 최신 Vercel 오류 로그에는 키 문자열이 `[REDACTED]`로 치환되어 기록됐습니다. 따라서 AI 성공 여부는 여전히 실패이고 로그 정화 동작만 PASS입니다.
